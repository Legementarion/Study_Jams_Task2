...
public class Animals extends Fragment {
    ...
    private String[] animalsMass = {"dog","lion","monkey"};
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        confirm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (editText.getText().toString().toLowerCase().equals(animalsMass[currentIndex])) {
                    currentIndex++;
                    if (currentIndex == animalsMass.length) {
                        menu.callOnClick();
                        hideKeybord();
                        Toast.makeText(getContext(), R.string.finish_massage, Toast.LENGTH_LONG).show();
                    } else {
                        loadImageFromAsset(animalsMass[currentIndex]);
                        editText.setText("");//hide keyboard
                        hideKeybord();
    ...
    public void loadImageFromAsset(String str) {
        try {
            InputStream ims = getActivity().getAssets().open("Animals/"+str+ ".jpg");
    ...
    private void randSort() {
        ...
        String temp;
        for (int i = 0, randIndex; i < animalsMass.length; i++) {
            temp = animalsMass[i];
            animalsMass[i] = animalsMass[randIndex = rnd.nextInt(animalsMass.length)];
            animalsMass[randIndex] = temp;
        }
    }
